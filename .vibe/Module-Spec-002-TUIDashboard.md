...
仅有 weekly 窗口的配额卡片也必须使用与其他 Coding Plan 一致的两行布局：第一行显示 Provider、进度条、剩余百分比和完整的 `RESET` 文案；第二行不伪造 `5H` 或其他不存在的窗口，只在共享状态列显示 `OK`、`STALE`、`AUTH` 或 `ERROR` 等状态徽标。这样状态徽标不会与重置文案争抢同一行，也不会被 60×20 网格截断。
...
- weekly-only 卡片处于 stale、auth 或 error 状态时，第一行完整保留主值和重置文案，第二行完整保留状态徽标，且不得互相粘连或截断。
